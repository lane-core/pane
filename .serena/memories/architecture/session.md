...
**Theoretical basis:** EAct single-mailbox invariant (Fowler/Hu Definition 3.1). DLfActRiS star topology (actor at center, connections as leaves) is trivially acyclic (Jacobs/Hinrichsen/Krebbers POPL 2024 Theorem 1.2 proves global progress).
...
**ConnectionSource** (highest priority): calloop EventSource wrapper for a single Connection, enables real Messenger/ServiceHandle routing (both read and write). pane-session provides the FrameCodec and reader/writer machinery; ConnectionSource would bridge to calloop's event loop.

---
name: Haiku source layout for key BeOS subsystems
description: Key file paths in ~/src/haiku for verifying BeOS architecture claims — app_server threading, messaging primitives, scripting protocol, translation kit
type: reference
---

Key Haiku source locations for BeOS architecture review:
...
- `headers/os/app/PropertyInfo.h` — property_info struct: the scripting protocol metadata (commands, specifiers, types, compound_types)
...
- Both ServerApp and ServerWindow have their own threads (MessageLooper pattern)
...
**BWindow inherits BLooper:** confirmed in `headers/os/interface/Window.h` line 93

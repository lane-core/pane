...
**par as substrate:** par is a direct dependency of pane-session. As of the redesign, par drives the protocol directly — handlers use par::exchange::Send::send() and par::exchange::Recv::recv() natively. No Chan<S,T> wrapper. pane-session provides bridge threads (spawned via par::fork_sync) that mediate between par's in-process oneshot channels and the Transport trait (postcard serialization for IPC). Transport panics on disconnect; par's .expect("sender dropped") propagates this as a panic to the handler.
